...
enum class Reason : std::uint8_t {
  ...
  kDefault,  // The resolved value fell back to a pre-configured value (no
             ...
             // result).
  kTargetingMatch,  // The resolved value was the result of a dynamic
                    // evaluation, such as a rule or specific user-targeting.
  kSplit,     // The resolved value was the result of pseudorandom assignment.
  kCached,    // The resolved value was retrieved from a cache.
  kDisabled,  // The resolved value was the result of the flag being disabled in
...
constexpr std::string_view ToString(Reason reason) noexcept {
  switch (reason) {
    case Reason::kStatic:
      return "STATIC";
    case Reason::kDefault:
      return "DEFAULT";
    case Reason::kTargetingMatch:
