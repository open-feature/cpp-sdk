...
class Flag{
  public:
    ...
    Flag(std::string type, std::string name, std::any default_value)
        : Flag(std::move(type), std::move(name), std::move(default_value), openfeature::FlagMetadata{}) {}

    Flag(std::string type, std::string name, std::any default_value, openfeature::FlagMetadata flag_metadata) 
    : name_(std::move(name)),
